...
def plot(avg_ensems, keys):
    markers = ['s','o'] if 'Pp' in keys else ['^','v']
    #fc = 'k' if 'Pp' in keys else 'none'
    fc = 'none'
    s = 175 if 'Diss' in keys else 150
    for n, key in enumerate(keys):
        param = avg_ensems[key]['a'][1:]
        param = [n/100 for n in param]
        gcont = G_contours[1:]
        errbar = [n/100 for n in avg_ensems[key]['r'][1:]]
        plt.scatter(gcont, param, marker=markers[n], facecolors=fc, edgecolors='k', s=150)
